...
let circleVertices = function buildCircleVertexBuffer() {
    ...
    for (let i = 0; i < CIRCLE_SEGMENT_COUNT; i++) {
        ...
        vertexData.push(0, 0, 0.200, 0.200, 0.957);
        vertexData.push(x1, y1, 0.200, 0.200, 0.957);
        vertexData.push(x2, y2, 0.200, 0.200, 0.957);
    }
...
//---------------------------------------------------------------------------------
class MovingShape {
    constructor(position, velocity, size, timeRemaining, vao, numVertices, force) {
...
function motionAndColor(width, height, scale, offsetX, offsetY) {
    ...
    //---------------------------------------------------------------------------------
    function createTwoBufferVertexAttribureObject(webGL2, positionBuffer, colorBuffer, positionAttributeLocation, colorAttributeLocation) {
    ...
    function createInterleaveBufferVertexAttribureObject(webGL2, interleaveBuffer, positionAttributeLocation, colorAttributeLocation) {
        const vertexAttributeObject = webGL2.createVertexArray();
        webGL2.bindVertexArray(vertexAttributeObject);
        webGL2.enableVertexAttribArray(positionAttributeLocation);
        webGL2.enableVertexAttribArray(colorAttributeLocation);
    ...
    let frame = function () {
        ...
        while (timetoNextSpawn < 0) {
            ...
            shapes.push(new MovingShape(position, velocity, size, timeRemaining, geometry.vao, geometry.numVertices, force));
        }
